...
Before emitting workstreams, restore any `codex-repo-state:v1` record on the selected Issue/PR with `@repo-handoff` and revalidate its base SHA. Include the chosen carrier, workflow ID, current state status, and the state-record update points in the plan. If no record exists, produce a bootstrap block; do not claim that planning state is durable until an authorized record is saved.
...
8. At every substantive checkpoint, prepare a `codex-repo-state:v1` update on the workstream's canonical Issue or PR, including the new SHA, status, gate, decision, and next action. Record it through `@repo-handoff` only when the coordinator has explicit authorization; otherwise return the exact block for the user to apply.
